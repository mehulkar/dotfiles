...
- **Don't escape backticks in heredoc-quoted PR bodies.** When passing a body via `gh pr create --body "$(cat <<'EOF' ... EOF)"`, the quoted `<<'EOF'` heredoc disables shell interpretation, so a backslash before a backtick is preserved literally and reaches GitHub as `\` — markdown renders the backslash and the code styling is lost. Write `` `foo` ``, not `` \`foo\` ``. Same applies to `$`, `"`, and other shell metacharacters; the quoted heredoc handles them.
